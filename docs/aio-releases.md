...
Use `aio-upgrade.yaml` to move a site to a newer `aioRelease`. It always bumps the AIO Arc extension. It upgrades secret-store only when the site declares `deployOptions.enableSecretSync: true`, and it upgrades cert-manager only when `deployOptions.enableCertManager: true`. Disabled optional extensions are neither resolved nor written, so they do not need to exist. Every extension that is updated preserves its existing `configurationSettings`, `releaseTrain`, and identity.
